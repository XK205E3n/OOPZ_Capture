...
- 将 Flash 价格修正和飞书费用提示纳入发布包；消息显示当前估算金额、模型及价格核验日期，不将用量缺失误报为价格缺失。
- 无配置或数据迁移。已完成任务可复用摘要刷新费用文本；不重发历史消息。仅在无录音/分析任务时更新，回滚恢复旧文字。
- 验证：价格及飞书费用提示专项检查通过；正式构建执行全量测试。
- 已发布与部署：2026-09-21 17:36 UTC 验证服务器运行 v0.11.15，提交 `3be0c95a5a5a200048e25e5139236dac00d27baf`；265 passed、1 skipped，审计通过。依赖下载缓慢后，按相同依赖声明复制既有运行库到独立新环境并验证，旧版保持可回滚。报告费用刷新未请求模型、未重发消息。

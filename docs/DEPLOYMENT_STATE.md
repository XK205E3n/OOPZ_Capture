...
| 项目 | 当前已知状态 | 验证边界 |
| --- | --- | --- |
| 本地/远端 | v0.11.10 发布包：`351fee9b773b`；在线下载文档为后续提交，运行代码一致 | 生产程序须从正式 ZIP 安装 |
...
| 启动 | 安装成功后启动网关；登录任务使用同一运行账户 | 无人登录和重启恢复尚未验收 |
...
新服务器实际安装的 Release ID、提交与完成时间尚未取得，不能登记为已验收。用户报告迁移后，浏览器只核对了实例规格及安全组；应用部署结果须依据服务器清单和端到端测试回填。

...
| 应用版本 | `0.11.4` | 待部署（含 README 修正的 GitHub Release 已生成） | 是 |
...
发布目录中的 `.env` 使用同盘硬链接指向 `shared\config\.env`；`models`、`output`、`feishu_state`、`logs` 使用目录联接指向 `shared`。因此代码回滚不会回滚或清空业务数据和配置。

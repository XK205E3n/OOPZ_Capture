...
用户报告飞书正文混入小时明细、PDF 失败且状态窗口未提示。main 已修复正文提取、PDF 实际浏览器选择/错误进度；本机 PDF 渲染通过，服务器记录为 PDF 渲染超时 180 秒；本机复现旧库异常后遗留 HTTP 服务的挂起路径，新渲染流程已移除该服务且真实 PDF 生成通过，服务器修复结果仍待升级后验收。云部署 `.env` 已备份并将思考设置为 enabled，其他字段未改；未直接更新服务器，发布仍暂停。

...
在管理员 PowerShell 中复制执行以下代码。它匿名下载当前 **v0.11.13** 正式 ZIP 和校验文件到 `C:\OOPZ\artifacts`，同时与这里固定的 SHA-256 比对，再验证清单和提取内容。已下载的同名文件放入该目录后会自动跳过下载；文件不符则停止，不覆盖。
...
function Get-OopzPinnedRelease {
    return @{
        Tag = 'v0.11.13'
        File = 'oopz-capture-v0.11.13-9bf093db9a6e.zip'
        Sha256 = '129a60d1943328bead4a6a39b67b977ca7bf789d60dab94eae695aa6e24a0e65'
        Commit = '9bf093db9a6eba4c4ae25dd7a730c388df8e2cf5'
        ReleaseId = 'v0.11.13-9bf093db9a6e'
    }
}
...
- [正式 ZIP](https://github.com/XK205E3n/OOPZ_Capture/releases/download/v0.11.13/oopz-capture-v0.11.13-9bf093db9a6e.zip)
- [SHA-256](https://github.com/XK205E3n/OOPZ_Capture/releases/download/v0.11.13/oopz-capture-v0.11.13-9bf093db9a6e.zip.sha256)
...
### 9.6 分析初始化报 segment_id must be a UUID

v0.11.11 及更早版本的静音分片会写入 `segment_id=no-speech`，与分析器的 UUID 契约冲突，可能导致整个会话在调用 API 前失败。v0.11.12 修复生成端，并兼容读取已存在的静音记录：只在内存中转换符合旧版完整特征的标记，不删除记录、不改写原转写文件，也不放宽普通转写记录的 UUID 校验。

按第 3 节获取 v0.11.12，已有服务器使用更新流程安装到新的 releases 目录并切换 current，保留 shared。确认运行版本后，在飞书发送“待分析”，选择原会话重试，不需重新录音或重新转写。尚有录音或分析任务时先等待任务结束；不要手改 current/releases 中的 Python 文件。

如果升级后仍在同一行报错，仅核查该行的 segment_id、transcript_source、language、agora_uid 等结构字段，避免发送整份会话；非静音标记的异常仍需要单独诊断。API 超时设置或更换模型无法解决输入 UUID 校验错误。

### 9.7 data_inspection_failed：短窗口拆分与跳过

v0.11.13 起，短窗口遭遇服务端 `data_inspection_failed` 时按时间中点拆分一次；正常 300 秒窗口变成两个 150 秒片段，尾部不足 300 秒的窗口按实际长度平分。每个非空半段分别请求，仍被同类审核拒绝则跳过该半段；不递归拆分，普通 HTTP 400、认证、超时等错误不会被静默跳过。拆分按时间范围选择原句，跨边界句子保留完整文字，不自动删词或改写原转写。

内部、公开及简版分析报告均注明缺失时间范围和审核原因；两半都被拒绝时整段标为未分析，不把缺失当作静音。原 transcript.jsonl/md 保留。成功短摘要缓存可复用，已完成拆分或跳过的结果也会缓存，重复运行不会反复请求它们。该机制不保证服务商审核放行，也不改变其审核规则；长摘要和最终综合遇到此错误仍停止并保留检查点。
...
每次执行会查询本仓库 GitHub 标记的 Latest 正式版，验证标签提交、GitHub 资产摘要、旁置 SHA-256 和包内清单，使用新包安装器升级，再核对 current。已是最新版则不重装、不重启；不会降级。本功能不是定时自动更新。

共享配置、模型和数据保留；不会自动覆盖 `.env`。如果新版配置契约变化，仍须先按发布说明补齐配置。安装可能较慢，切换时会重启网关；健康检查失败由目标版本安装器尝试回滚。任务检查在准备前和安装前执行，但无法阻止之后新发起的飞书任务。

若目标版本目录已存在，代码会停止，保留上次安装现场；不要删除目录或反复执行安装。GitHub API 限流、资产摘要缺失或校验不一致时也停止，可使用第 3 节固定版本流程。首次安装、current 丢失或损坏不适用本节。

<!-- update-latest-copy:start -->
```powershell
& {
...
```
<!-- update-latest-copy:end -->

看到 `UPDATED` 后，在飞书检查“状态”，按第 10 节验收一次录音、转写和分析。看到 `ALREADY_LATEST` 表示版本一致，不等同于服务健康检查通过。程序升级不会重录、重转写或自动重新分析旧会话，需要时在飞书发送“待分析”。

脚本也保存在 `scripts/update_latest_release.ps1`；下载该脚本后可加 `-PrepareOnly` 仅下载校验、准备管理脚本，不调用安装器、不切换 current。默认安装根目录为 `C:\OOPZ`，自定义目录用 `-InstallRoot` 传入。

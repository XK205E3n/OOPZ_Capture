"""One-off QQ send requests produced by the controller for the gateway.

The controller and the gateway run as separate processes; the controller
cannot call OneBot actions itself.  Admin-initiated report forwarding writes
a send request into ``state_root/send_requests/`` and the gateway polls,
executes and acknowledges it.
"""
...
SEND_REQUEST_SCHEMA = "oopz.qq.send_request.v1"
...
def enqueue_send_request(
    ...
    *,
    ...
    if not target_id.isascii() or not target_id.isdigit() or not 5 <= len(target_id) <= 20:
        raise ValueError("target_id must contain 5 to 20 ASCII digits")
    ...
    if notify_admin_value is not None and (
        not notify_admin_value.isascii() or not notify_admin_value.isdigit() or not 5 <= len(notify_admin_value) <= 20
    ):
        raise ValueError("notify_admin_id must contain 5 to 20 ASCII digits")
...
def reschedule_send_request(
    ...
    *,
    ...
    """Persist a bounded retry instead of losing a notification on a transient QQ error."""
    path = _request_path(state_root, request_id)
    if not path.is_file() or _is_reparse_point(path):
        raise ValueError(f"send request not found: {request_id}")
    value = json.loads(path.read_text(encoding="utf-8"))
...
def expedite_pending_send_requests(state_root: Path) -> int:
    """Make retained transient notifications eligible immediately after QQ recovers."""
    changed = 0

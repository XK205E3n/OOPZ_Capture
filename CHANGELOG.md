...
### 2026-09-21 — 修复：飞书费用提示同步最新 Flash 计价

- 飞书消息明确显示“预估费用”、DeepSeek Flash、北京时间峰谷规则及价格核验日期；区分未匹配价格与审核拒绝请求缺少用量，避免误报“没有已核验单价”。
- 将下列 Flash 价格修正纳入正式发布包；更新后可复用已完成摘要重新渲染费用说明，无需重新请求模型，不重发已发送消息。
- 验证：费用提示、已核验费率及缺失用量提示专项检查通过；无新增配置或数据迁移，回滚恢复旧费用文字。
- 发布验证：265 passed、1 skipped，审计通过；服务器已部署 v0.11.15 并恢复飞书连接。依赖声明一致，沿用原运行库的独立副本；最新报告只刷新费用和 PDF，摘要哈希不变、无新增模型请求。
